...
// ═══════════════════════════════════════════════════════════
// AURORA BOREALIS FLUID SHADER — Premium, organic, no meshes
// Pure GPU-driven GLSL light simulation with mouse reactivity
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
// AURORA BACKGROUND PLANE (touch + mouse compatible)
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
// 3D SPATIAL NAVIGATION WAYPOINTS — Floating orbs in 3D
// Replace traditional navbar with immersive spatial markers
// ═══════════════════════════════════════════════════════════
    ...
    <group ref={groupRef}>
      {sections.map((section, i) => (
        <group key={i} position={section.position.toArray()}>
          ...
          >
            <span
              className="text-[10px] tracking-[0.3em] uppercase font-light opacity-40"
              style={{ fontFamily: "var(--font-raleway), sans-serif", color: section.color }}
            >
              {section.label}
            </span>
          </Html>
...
// ═══════════════════════════════════════════════════════════
// SPATIAL CAMERA CONTROLLER — Smooth fly-through on scroll
// ═══════════════════════════════════════════════════════════
function CameraController() {
  ...
  useFrame((state) => {
    ...
    // Smooth fly-through Z positions matching section depths
...
// ═══════════════════════════════════════════════════════════
// CSS KEYFRAME STYLES — For scroll-triggered text animations
// ═══════════════════════════════════════════════════════════
const sceneStyles = `
...
// ═══════════════════════════════════════════════════════════
// ELEGANT SPATIAL HTML UI — CSS-animated text reveals
// ═══════════════════════════════════════════════════════════
      ...
      <div className="w-screen h-[300vh] relative pointer-events-none">
        ...
        <section className="absolute top-0 left-0 w-screen h-screen flex flex-col justify-end p-8 md:p-16">
          <div className="pointer-events-auto pb-12 w-full max-w-5xl">
            {/* Elegant serif display — Playfair Display */}
            <h2 
              className="anim-reveal-1 text-[11vw] md:text-[7rem] font-bold italic leading-[0.85] tracking-tight bg-gradient-to-r from-[#D7FF00] via-[#00D9FF] to-[#D7FF00] bg-clip-text text-transparent"
              style={{ fontFamily: "var(--font-playfair), serif", WebkitBackgroundClip: "text" }}
            >
              Vision.
            </h2>
            <h2 
              className="anim-reveal-2 text-[11vw] md:text-[7rem] font-extrabold leading-[0.85] tracking-tight bg-gradient-to-r from-white/90 via-[#00D9FF] to-white/90 bg-clip-text text-transparent -mt-1"
              style={{ fontFamily: "var(--font-raleway), sans-serif", WebkitBackgroundClip: "text" }}
            >
              Craft.
            </h2>
            ...
            {/* Copy block */}
            <p 
              className="anim-reveal-4 text-white/80 text-base md:text-lg leading-relaxed max-w-xl mt-6 pl-6 font-light"
              style={{ fontFamily: "var(--font-raleway), sans-serif", borderLeft: "2px solid", borderImage: "linear-gradient(to bottom, #D7FF00, transparent) 1" }}
            >
        ...
        <section className="absolute top-[100vh] left-0 w-screen h-screen flex items-center justify-center p-4 md:p-12">
          <div className="pointer-events-auto w-full max-w-[90vw]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-1">
              ...
              {/* Tier 1: Ember */}
              <div 
                className="relative group p-8 md:p-10 flex flex-col justify-between min-h-[320px] overflow-hidden transition-all duration-500 hover:-translate-y-3 active:-translate-y-3 backdrop-blur-sm"
                style={{ background: "linear-gradient(135deg, rgba(255,0,60,0.08) 0%, rgba(5,5,5,0.9) 40%, rgba(5,5,5,0.95) 100%)", borderTop: "1px solid rgba(255,0,60,0.3)", WebkitTapHighlightColor: "transparent" }}
              >
              ...
              {/* Tier 2: Neon */}
              <div 
                className="relative group p-8 md:p-10 flex flex-col justify-between min-h-[320px] md:-translate-y-6 overflow-hidden transition-all duration-500 hover:-translate-y-10 active:-translate-y-8"
                style={{ background: "linear-gradient(135deg, rgba(215,255,0,0.15) 0%, rgba(0,217,255,0.08) 50%, rgba(215,255,0,0.1) 100%)", borderTop: "2px solid #D7FF00", borderBottom: "2px solid #00D9FF", WebkitTapHighlightColor: "transparent" }}
              >
              ...
              <div 
                className="relative group p-8 md:p-10 flex flex-col justify-between min-h-[320px] overflow-hidden transition-all duration-500 hover:-translate-y-3 active:-translate-y-3 backdrop-blur-sm"
                style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.03) 0%, rgba(5,5,5,0.9) 40%, rgba(5,5,5,0.95) 100%)", borderTop: "1px solid rgba(255,255,255,0.15)", WebkitTapHighlightColor: "transparent" }}
              >
        ...
        <section className="absolute top-[200vh] left-0 w-screen h-screen flex items-center justify-center p-8 overflow-hidden">
           ...
           <div className="pointer-events-auto text-center relative z-10 px-8">
              ...
              <h2 
                className="text-[14vw] md:text-[10rem] font-bold italic leading-none cursor-pointer tracking-tight transition-all duration-500 hover:tracking-wide"
                style={{ fontFamily: "var(--font-playfair), serif", backgroundImage: "linear-gradient(135deg, #D7FF00, #00D9FF, #D7FF00)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}
              >
                100/100
              </h2>
              ...
              <div className="mt-8 flex flex-col items-center gap-8">
                ...
                {/* Gradient CTA */}
                <button 
                  className="relative overflow-hidden text-lg md:text-xl px-10 py-4 uppercase tracking-[0.2em] font-semibold transition-all duration-300 hover:-translate-y-1 active:scale-95 active:brightness-110 group"
                  style={{ fontFamily: "var(--font-raleway), sans-serif", background: "linear-gradient(135deg, #D7FF00, #00D9FF)", color: "#050505", WebkitTapHighlightColor: "transparent" }}
                >
...
// ═══════════════════════════════════════════════════
// MAIN SCENE — Aurora + Spatial Nav + Scroll UI
// ═══════════════════════════════════════════════════
export default function Scene() {
  ...
  return (
    <div 
      className="fixed inset-0 z-0 h-screen w-screen bg-[#030305] overflow-hidden"
      style={{ touchAction: 'none', WebkitOverflowScrolling: 'touch' } as React.CSSProperties}
    >
      <Canvas 
      ...
      >
        ...
        <ScrollControls pages={3} damping={isMobile ? 0.3 : 0.2}>
          <CameraController />
          {/* 3D Spatial waypoints — floating orbs as navigation markers */}
          <SpatialWaypoints />
          <SpatialHTMLUI />
        </ScrollControls>
      </Canvas>
