    ...
    <div className="relative h-96 w-full max-w-sm rounded-xl bg-onyx/40 border border-white/10 overflow-hidden flex flex-col backdrop-blur-sm group">
      <div className="p-4 border-b border-white/5 bg-white/5 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <Flame className="text-siren-red" size={16} />
          <span className="text-xs font-bold text-white tracking-widest">MEMETIC_SIMULATOR</span>
        </div>
        <div className="flex gap-2">
          <button 
            onClick={() => setIsRunning(!isRunning)}
            className={`p-1.5 rounded hover:bg-white/10 transition-colors ${isRunning ? 'text-signal-lime' : 'text-white/60'}`}
          >
      ...
      <div className="p-6 flex-1 flex flex-col gap-6">
          ...
          {grid.map((state, i) => (
            <motion.div
              ...
              className={`rounded-sm cursor-pointer transition-colors duration-300 ${
                state === "infected" ? "bg-siren-red shadow-[0_0_8px_rgba(239,68,68,0.8)]" :
                state === "recovered" ? "bg-blue-500/20" :
                "bg-white/5 hover:bg-white/10"
              }`}
              animate={{
                scale: state === "infected" ? [1, 1.2, 1] : 1,
              }}
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <Flame size={12} className="text-siren-red" />
            <input 
              type="range" 
              min="0" max="1" step="0.1"
              value={virality}
              onChange={(e) => setVirality(parseFloat(e.target.value))}
              className="flex-1 h-1 bg-white/10 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-siren-red"
            />
            <span className="text-[10px] font-mono w-8 text-right text-siren-red">VIR:{virality}</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <input 
              ...
              className="flex-1 h-1 bg-white/10 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-blue-400"
            />
             <span className="text-[10px] font-mono w-8 text-right text-blue-400">RES:{resistance}</span>
          </div>
        </div>
      </div>
    </div>
