...
const CHANNELS = [
  ...
  { id: "em", icon: Mail, color: "text-signal-lime", bg: "bg-signal-lime" },
];
    ...
    <div className="relative h-96 w-full max-w-sm rounded-xl bg-onyx/40 border border-white/10 overflow-hidden flex flex-col backdrop-blur-sm">
      <div className="p-4 border-b border-white/5 bg-white/5 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <AudioWaveform className="text-signal-lime" size={16} />
          <span className="text-xs font-bold text-white tracking-widest">RHYTHM_SEQUENCER</span>
        </div>
        <button
            ...
            <div className="flex-1 grid grid-cols-8 gap-1.5">
              {sequence[rowIdx].map((isActive, colIdx) => (
                <motion.button
                  ...
                  className={`h-6 rounded-sm transition-all relative overflow-hidden ${
                    isActive ? channel.bg : "bg-white/5"
                  } ${colIdx === currentStep && isPlaying ? "brightness-150 ring-1 ring-white/50" : ""}`}
                  whileTap={{ scale: 0.9 }}
                >
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-1">
             {[...Array(3)].map((_, i) => (
               <motion.div 
                 key={i}
                 className="w-1 h-3 rounded-full bg-signal-lime/50"
